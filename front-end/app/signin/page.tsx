...
import { Brain, ArrowLeft, Sparkles, Shield, Zap, Star, CheckCircle, Eye, EyeOff, Lock } from "lucide-react"
  ...
  const handleCodeVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.valid) {
        // Code is valid, set authentication
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 sm:p-10 shadow-2xl">
            {step === "signin" ? (
              ...
              <>
                ...
                <div className="text-center mb-8">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-r from-green-500 to-blue-500 flex items-center justify-center mx-auto mb-6 shadow-lg shadow-green-500/25">
                    <Lock className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
                  </div>
                  <h1 className="text-2xl sm:text-3xl font-bold text-white mb-3">Enter Access Code</h1>
                  <p className="text-gray-400 text-sm sm:text-base">Please enter your access code to continue</p>
                </div>
                ...
                {/* Code Input Form */}
                <form onSubmit={handleCodeVerification} className="mb-6">
                  <div className="relative mb-6">
                    <input
                      ...
                      className="w-full py-4 px-6 pr-12 bg-white/10 border border-white/20 rounded-2xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-all duration-300"
                      required
                      disabled={isLoading}
                    />
                    <button
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-medium py-4 px-6 rounded-2xl transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                  >
                    {isLoading ? (
                      <>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full text-gray-400 hover:text-white transition-colors text-sm py-2 disabled:opacity-50"
                >
                  ← Back to Sign In
                </button>
              </>
